...
classdef INV_Square <handle
      ...
      methods(Static)
        function [Name,SaveName]=Instruct()
            Name='BP=m*1/(PAT^2)+c';
            SaveName='Evaluated_BP_INVSQUARE.mat';
        end
      end
    
    methods
        function obj =INV_Square()
            obj.S_Par=[];
            obj.D_Par=[];
        end
        ...
        function [estSBP,estDBP] = Predict(obj,PAT)
            estSBP=(obj.S_Par(1)*1./(PAT.^2)+obj.S_Par(2));
            estDBP=(obj.D_Par(1)*1./(PAT.^2)+obj.D_Par(2));
        end
        
        function [SBPMsg,DBPMsg]=Printmodel(obj)
            SBPMsg=['SBP=' num2str(obj.S_Par(1),3) '*1/(PAT^2)+' num2str(obj.S_Par(2),3)];
            DBPMsg=['DBP=' num2str(obj.D_Par(1),3) '*1/(PAT^2)+' num2str(obj.D_Par(2),3)];
        end
    end
end
