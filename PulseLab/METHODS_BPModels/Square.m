...
classdef Square <handle
      ...
      methods(Static)
        function [Name,SaveName]=Instruct()
            Name='BP=m*(PAT^2)+c';
            SaveName='Evaluated_BP_SQUARE.mat';
        end
      end
    ...
    methods
        ...
        function [estSBP,estDBP] = Predict(obj,PAT)
            estSBP=(obj.S_Par(1)*(PAT.^2)+obj.S_Par(2));
            estDBP=(obj.D_Par(1)*(PAT.^2)+obj.D_Par(2));
        end
        
        function [SBPMsg,DBPMsg]=Printmodel(obj)
            SBPMsg=['SBP=' num2str(obj.S_Par(1),3) '*(PAT^2)+' num2str(obj.S_Par(2),3)];
            DBPMsg=['DBP=' num2str(obj.D_Par(1),3) '*(PAT^2)+' num2str(obj.D_Par(2),3)];
        end
    end
end
