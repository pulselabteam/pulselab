...
classdef Log<handle
    ...
    methods
        ...
        function [estSBP,estDBP] = Predict(obj,PAT)
            estSBP=(obj.S_Par(1)*log(PAT)+obj.S_Par(2));
            estDBP=(obj.D_Par(1)*log(PAT)+obj.D_Par(2));
        end
        
        function [SBPMsg,DBPMsg]=Printmodel(obj)
            SBPMsg=['SBP=' num2str(obj.S_Par(1),3) '*log(PAT)+' num2str(obj.S_Par(2),3)];
            DBPMsg=['DBP=' num2str(obj.D_Par(1),3) 'log(PAT)+' num2str(obj.D_Par(2),3)];
        end
    end
end
