...
classdef ECG_sSQI<handle
    ...
    methods(Static)
        ...
        function power=FPower(SIG,FL,FH,FSMP)
            L=length(SIG);
            if ~mod(L,2)
                L=L-1;
                SIG(end)=[];
            ...
            idxL=find(f>FL,1,'first');
            idxH=find(f<FH,1,'last');
            band=Y(idxL:idxH);
            power=band'*band;
        end
    end
    methods
        ...
        function obj = ECG_sSQI(Low,High,varargin)
            obj.Low=Low;
            obj.High=High;
        end
        function [accept,idx,SQI]=Execute(obj,SignalObject)
            SQI=obj.FPower(SignalObject.Sig,5,14,SignalObject.RATE)/obj.FPower(SignalObject.Sig,5,50,SignalObject.RATE);
